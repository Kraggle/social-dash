...
class AppHelper {
     ...
     * @return string
     */
    public static function makeSetting($setting, bool $echo = false) {
        ...
        <div class="row">
            ...
            <?php if ($type == 'checkbox') {
                $default_value = AppHelper::isTrue(old($key) ?? $options->default ?? 0);
                $has_cost = $options->on_cost || $options->off_cost;
                $cost = $default_value ? $options->on_cost ?? 0 : $options->off_cost ?? 0; ?>

                <div class="col has-switch mb-2">
                    <input class="bootstrap-switch" type="checkbox" name="<?= $key ?>" <?= AppHelper::checked($default_value) ?> data-off-label="NO" data-on-label="YES" data-on-cost="<?= $options->on_cost ?>" data-off-cost="<?= $options->off_cost ?>" data-default-cost="" has-cost>
                </div>

            <?php } elseif ($type == 'text') {
                $default_value = old($key);
                foreach ($options->values as $option) {
                    if (!$default_value && $option->default)
                ...
                <div class="col pr-0 mb-2">
                    <select name="<?= $key ?>" class="selectpicker" data-size="7" data-style="btn btn-primary" data-default-cost="" has-cost>
                        <?php foreach ($options->values as $option) { ?>
                            <option value="<?= $option->value ?>" data-cost="<?= $option->cost ?>" <?= AppHelper::selected($default_value, $option->value) ?> data-content="
            ...
            <?php } elseif ($type == 'number') {
                ...
                $step = $options->step ?? 1;
                $default_value = old($key) ?? $options->default ?? $min;
                $per = (($default_value - $min) / ($max - $min));
                ...
                <div class="col-sm-2 pr-0 mb-2">
                    <div class="input-group">
                        ...
                        <input type="number" class="form-control pl-2 pr-0" id="<?= $key ?>" name="<?= $key ?>" min="<?= $min ?>" max="<?= $max ?>" step="<?= $step ?>" value="<?= $default_value ?>" />
                    </div>
                </div>

                <div class="col mb-2">
                    <div id="noUi_<?= $key ?>" name="<?= $key ?>" class="slider slider-primary" data-min="<?= $min ?>" data-max="<?= $max ?>" data-min-cost="<?= $cost_min ?>" data-max-cost="<?= $cost_max ?>" data-step="<?= $step ?>" data-default="<?= $default_value ?>" style="margin-top: 18px;"></div>
                </div>
        ...
        if ($hidden) {
            ob_start(); ?>
            <input type="hidden" name="<?= $key ?>" value="<?= $default_value ?>" data-cost="<?= $cost ?>">
